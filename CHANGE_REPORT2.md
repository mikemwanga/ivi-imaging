...
3. Loading pipeline replaced with sequential projection
...
5. Signal calculation behavior changed
...
6. Binning API and behavior changes
...
7. Scale-bar functionality removed
...
8. Population creation behavior changed
...
## Notes for the forked repo
- The core behavioral change is the sequential processing in `load_and_project`, which avoids keeping all images in GPU/CPU memory at once.
- The modified notebook aligns with the new API and changed column naming in `calculate_cell_signals` and `bin_cell_signal`.
